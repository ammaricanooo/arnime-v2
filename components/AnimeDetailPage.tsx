...
export default function AnimeDetailPage({ slug }: { slug: string }) {
    ...
    const handleShare = async () => {
        ...
        if (navigator.share) {
            await navigator.share({
                title: detail?.title,
                text: `Nonton ${detail?.title} sub indo di Arnime`,
                url,
            })
        } else {
    ...
    const handleWatchEpisode = async (epSlug: string, epTitle: string) => {
        ...
        if (user && detail) {
            try {
                await setDoc(
                    doc(db, "history", `${user.uid}_${slug}`),
                    {
                        userId: user.uid,
                        slug,
                        title: detail.title,
        ...
        <div className="space-y-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-8">
                <div className="md:col-span-1">
                    <img
                        src={detail.poster || "/placeholder.svg"}
                        alt={detail.title}
                        className="w-full min-w-40 rounded-lg shadow-lg object-cover aspect-3/4"
                    />
                    ...
                    <div className="flex gap-3 mt-4">
                        <button
                            ...
                            className={`flex-1 flex items-center justify-center gap-2 px-4 py-2 rounded-lg transition-all font-medium ${isLiked ? "bg-red-600 text-white" : "bg-indigo-600 text-white"
                        ...
                        <button
                            onClick={handleShare}
                            className="flex-1 flex items-center justify-center gap-2 px-4 py-2 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-slate-100 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors font-medium"
                        >
            ...
            <div className="block md:hidden">
                <div className="mb-4">
                    <h2 className="text-xl font-semibold text-slate-900 dark:text-slate-100 mb-2">Synopsis</h2>
                    <p className="text-slate-700 dark:text-slate-300 leading-relaxed">
                        {detail.synopsis || 'No synopsis available.'}
                    </p>
                </div>
